...
async def generate_spek(_, message: Message):
    ...
    else: return await message.reply_text("Can only generate spectrum from audio file....", quote=True)

    file_name = str(media.file_name)
    mime = media.mime_type
    if message.media.value == "document" and "audio" not in mime:
        return await message.reply_text("Can only generate spectrum from audio file....", quote=True)
